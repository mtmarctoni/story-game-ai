...
export function GameSidebar({ settings, editable = false, isLoading = false, onChange }: GameSidebarProps) {
  const [collapsed, setCollapsed] = useState(false);
  return (
      <aside className={`transition-all duration-300 ${collapsed ? 'w-12 min-w-[48px]' : 'w-full md:w-96'} px-0 py-4 border-r flex flex-col h-full shadow-lg bg-gradient-to-b from-gray-900 to-gray-800 relative`}>
        <button
        ...
        >
          {collapsed ? <BookOpen size={20} /> : <span className="text-lg">&laquo;</span>}
        </button>
      <div className="sticky top-0 z-10 bg-gray-900 px-6 pb-2 pt-4 border-b border-gray-800 flex items-center gap-2">
          ...
          {!collapsed && <span className="text-lg font-bold text-blue-300 tracking-tight">Story Settings</span>}
      </div>
        {!collapsed && (
